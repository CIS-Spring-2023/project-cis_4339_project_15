...
export default {
  ...
  methods: {
    ...
    async submitEditService() {
      try {
        const response = await axios.put(`${apiURL}/services/${this.SelectedService}`, {
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-10">
          ...
          <div class="flex flex-col">
            <label class="block">
              ...
              <select v-if="services" v-model="SelectedService"
                class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                <option value="" disabled selected>Select a service</option>
                <option v-for="(item, index) in services" :key="index" :value="item._id">{{ item.serviceName }}, {{
                  item.serviceNumber }} </option>
              </select>
